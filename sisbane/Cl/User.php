...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO usuario (usuario, email, clave, nivel,created) VALUES ( '$name', '$email', '$password', '$nivel',CURRENT_TIMESTAMP)";
	...
	public function banuser( array $data )
	{
		if( !empty( $data ) ){
			...
			// escapar de las variables para la seguridad
			$idb = mysqli_real_escape_string( $this->_con, $trimmed_data['idb'] );
			
			//$nivel= $_POST['group1'];
			
			
			//$password = md5( $password );
			$query = "DELETE FROM `usuario` WHERE `usuario`.`id_usuario` = '$idb'";
			if(mysqli_query($this->_con, $query)){
				mysqli_close($this->_con);
				return true;
			};
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT  id_usuario, usuario, email, clave, fotoperfil, nivel, estatus FROM usuario where email = '$email' and clave = '$password' and estatus='ACTIVO' ";
	...
	public function account( array $data )
	{
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "UPDATE usuario SET clave = '$password' WHERE id_usuario = '$user_id'";
			if(mysqli_query($this->_con, $query)){
				mysqli_close($this->_con);
				return true;
			}
	...
	public function forgetPassword( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "UPDATE usuario SET clave = '$password1' WHERE email = '$email'";
			if(mysqli_query($this->_con, $query)){
				mysqli_close($this->_con);
				$to = $email;
				$subject = "Nueva solicitud de contraseña";
				$txt = "Su nueva contraseña ".$password;
				$headers = "From: soportelh@holklegion.com" . "\r\n" .
						"CC: soportelh@holklegion.com";
	...
	public function registrationbit( array $data )
	{
		if( !empty( $data ) ){
			...
			// escapar de las variables para la seguridad
			$name = mysqli_real_escape_string( $this->_con, $trimmed_data['iduser'] );
			$id_facebook = mysqli_real_escape_string( $this->_con, $trimmed_data['idfacebook'] );
			$bitacora = mysqli_real_escape_string( $this->_con, $trimmed_data['bitacora'] );
			$pregun= $_POST['pregun'];
		
			$query = "INSERT INTO baneados (id_facebook, causa, ban_anterior, fecha_ban, id_usuario) VALUES ( '$id_facebook', '$bitacora', '$pregun',CURRENT_TIMESTAMP, '$name')";
			if(mysqli_query($this->_con, $query)){
				mysqli_close($this->_con);
				return true;
			};
		} else{
			throw new Exception( USER_REGISTER_FAIL );
		}
	}
